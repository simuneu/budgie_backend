...
public class EmailController {
    ...
    //인증코드 발송
    @PostMapping("/send")
    public ResponseEntity<ApiResponse<Void>> sendVerificationEmail(@Valid @RequestBody EmailRequestDto requestDto){
        authService.sendVerificationEmail(requestDto.getEmail());

        return ResponseEntity.ok(ApiResponse.okMessage("이메일이 성공적으로 발송되었습니다."));
    }

    //인증코드 검증
    @PostMapping("/verify")
    public ResponseEntity<ApiResponse<Void>> verifyEmailCode(@Valid @RequestBody EmailVerificationRequestDto requestDto){
        authService.completeEmailVerification(requestDto.getEmail(), requestDto.getCode());
        return ResponseEntity.ok(ApiResponse.okMessage("이메일 인증 완료"));
    }
}
