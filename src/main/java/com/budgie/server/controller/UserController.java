...
public class UserController {
    ...
    @GetMapping("/me")
    public ResponseEntity<ApiResponse<UserInfoDto>> getMyInfo(Principal principal){
    ...
    //닉네입 변경
    @PutMapping("/nickname")
    public ResponseEntity<ApiResponse<Void>> changeNickname(Principal principal, @RequestBody NicknameChangeRequestDto dto){

        Long userId = Long.parseLong(principal.getName());
        userService.changeNickname(userId, dto.getNickname());

        return ResponseEntity.ok(ApiResponse.okMessage("비밀번호가 성공적으로 변경되었습니다."));
    }
}
